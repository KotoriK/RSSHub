...
module.exports = async (ctx) => {
    ...
    if (StatisticsRaw && AreaStatRaw) {
        ...
        const oldData = (function(dataPath, newestDate) {
            ...
            if (file) {
                ...
                if (historyData.length === undefined) {
                    ...
                    historyPushes=historyData[2]?historyData[2].historyPushes:[];
        ...
        historyPushes=historyPushes.filter((value)=>{
            return value.pubDate!==toPush.pubDate;
        })
        if(historyPushes.unshift(toPush)>HISTORY_PUSHES_MAX){
            do{historyPushes.pop();}
            while(historyPushes.length>HISTORY_PUSHES_MAX);
        };
        for(const i in historyPushes){
            items.push(historyPushes[i]);
        }
        // 覆写
        fs.writeFile(
            dataPath,
