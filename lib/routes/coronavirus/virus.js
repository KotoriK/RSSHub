...
module.exports = async (ctx) => {
    ...
    // console.time("read")
    let StatisticsRaw, AreaStatRaw, PV;
    ...
    if (StatisticsRaw && AreaStatRaw) {
        ...
        if (historyPushes.unshift({
                ...
                <br>
                ${Statistics.countRemark}
                <br>
                    ${Statistics.confirmedIncr ? `<br>较昨日变化:确诊${Statistics.confirmedIncr}例 疑似${Statistics.suspectedIncr}例 重症${Statistics.seriousIncr}例 治愈${Statistics.curedIncr}例 死亡${Statistics.deadIncr}例` : ''
                    }
                ...
                ${(function (dataProvince, oldDataProvince) {
                        // 处理传入参数const cheerio = require('cheerio');
const fs = require('fs');
const path = require('path');
const dataPath = path.join(path.resolve('./'), 'tmp', 'virus_data.json');
const srcLink = 'https://3g.dxy.cn/newh5/view/pneumonia';
...
module.exports = async (ctx) => {
    ...
    if (StatisticsRaw && AreaStatRaw) {
        ...
        if (historyPushes.unshift({
            ...
            description: `<pre>
                            ...
                            if (now[subscribedProvinces[item]]) {
                                ...
                                for (const dataName in nowDataProccessed) {
                                    if (dataName !== 'unknown') {
                                        ...
                                        strArray.push(`
                ${dataName}: $ {
                    nowDataProccessed[dataName]
                }
                ${differ[dataName] === 0 ? '' : `(${addModifySign(differ[dataName])})`} 例 `);
                                    }
                                }
                ...
                ${/**<br>疫情发展情况:<img src=${map} referrerpolicy="no-referrer">*/''}
                ${/* (function (pic){
    ...
    // console.timeEnd('run time')
                ...
                $ {
                    differ[dataName] === 0 ? '' : `(${addModifySign(differ[dataName])})`
                }
                例 `);
                                        }
                                    }
                                    if (nowDataProccessed.unknown) {
                                        strArray.push(`($ {
                    nowDataProccessed.unknown
                })
                `);
                                    }
                                    str += ` < br > $ {
                    subscribedProvinces[item]
                }: $ {
                    strArray.join('，')
                }
                `;
